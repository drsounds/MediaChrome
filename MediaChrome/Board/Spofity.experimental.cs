...
[Serializable]
    ...
    /// <returns></returns>
        ...
        public Element RenderElements(Section srcSection,ref  Element C, XmlElement iSection)
        {
            ...
            foreach (XmlNode node in items)
            {
                ...
                /**
                 * If the element is not an child of the C element in XML
                 * data, skip it
                 * */

                if (node.ParentNode != iSection)
                    continue;
           ...
           //     ExtractXml(item, CT);
                ...
                C.Elements.Add(CT);
                RenderElements(srcSection,ref CT, item);
                
                // Raise element render element
                this.ParentBoard.RaiseElementParseEvent(CT);
    ...
    /// <returns></returns>
        ...
        public Section RenderSection(Section C, XmlElement iSection)
        {
            ...
            foreach (XmlNode node in items)
            {
                ...
                XmlElement item = (XmlElement)node;
                /**
                 * Do not add the element unless
                 * the element's parent is items
                 * */
                if (node.ParentNode != iSection)
                    continue;
                // Create various kind of controls depending of node type
	...
	public class Section
	{
	    ...
	    public void AddElement(Element d,Spofity X)
	    {
            ...
            // Raise add element event on parent board
	...
	{
        /// <summary>
        /// Anchors to the element
        /// </summary>
        public System.Windows.Forms.AnchorStyles Anchors { get; set; }
        ...
        {
            this.AssertBounds(false);
            return new Rectangle(this.Left-scrollX,this.Top+scrollY,Width,Height);
        
        //    throw new Exception("This method has been depredicted");
...
#if(nobug)
            // if element has an parent set according to the parent
            if (_Element.Parent != null)
            {
                ...
                {
                    bool foundYourself=true;
                    width = Bounds.Width - padding*2;
                    left = padding;
                    int __topPos=this.Top + padding;
                    foreach (Element cf in _Element.Parent.Elements)
                    {
                        int aheight = 0; // the width of the current element
                        /*
                         ...
                         * */
                        String _height = cf.GetAttribute("height");
                        if (_height.EndsWith("%"))
                        {
                            float c = float.Parse(_height.Replace("%", "")) / 100;
                            aheight = (int)((float)c * (height- padding*2));
                        }
                        else
                        {
                            aheight = int.Parse(_height.Replace("px", ""));
                        }
                        __topPos += aheight;
                        if (foundYourself)
                        {
                ...
                {
                    bool foundYourself=false; // Has found yourself in the collection
                    int __leftpos = _Element.Parent.Left;
                    height = _Element.Parent.Height;
                    
                    foreach(Element cf in _Element.Parent.Elements)
                    {
                        int awidth = 0; // the width of the current element
                        /*
                         *  If we come to yourself, break and begin assigning
                         *  */
                        if(cf == this)
                        {
                             ...
                             * Compute width of the element
                             * */
                            String _width = cf.GetAttribute("width");
                            if(_width.EndsWith("%"))
                            {
                            ...
                            {
                                awidth=int.Parse(_width.Replace("px",""));
                            }
                            __leftpos+=awidth;
                        if(foundYourself)
                        {
                           this.Width = __leftpos - Left;
                        }
                    }
     ...
     * */
            ...
            if (this.GetAttribute("noelm") != "")
                return;
            // If the element has an parent do not invoke this method
            int padding = 1;
            int top = 0;

            int left = this.Left, width = this.Width, height = this.Height;
            ...
            // Get if the element is persistent
            bool persistent = false;
            bool.TryParse(GetAttribute("persistent"), out persistent);
            Persistent = persistent;

            /**
             * If element is below an hbox/vbox set bounds relative
             * to it and it's other children
             * */


            /***
            * Set anchors for the element. Default is Anchors.Left
            * */
            String anchors = GetAttribute("anchors");
            this.Anchors = AnchorStyles.Left;
            ...
            {
                this.Anchors |= AnchorStyles.Right;
            }
            if (anchors.Contains("b"))
            {
                this.Anchors |= AnchorStyles.Bottom;
            }
            if (anchors.Contains("t"))
            {
                this.Anchors |= AnchorStyles.Top;
            }
            Element _Element = this;
            // Align elements by anchor
            switch (this.Anchors)
            {
                case AnchorStyles.Left | AnchorStyles.Right:
                    width = Bounds.Width - (Width - (this.Left + _Element.Width));
                    break;
                case AnchorStyles.Top | AnchorStyles.Bottom:
                    height = Bounds.Height - (Height - (_Element.Top + _Element.Height));
                    break;
                case AnchorStyles.Left:

                    break;
                case AnchorStyles.Right:
                    left = (Bounds.Width - (Width - (_Element.Left + _Element.Width))) - _Element.Width;
                    break;
                case AnchorStyles.Bottom:
                    height = (Bounds.Height - (Height - (_Element.Top + _Element.Height))) - _Element.Height;
                    break;
            }
            ...
            /**
             * Set width and height according to the parents bound
             * if not the position is set to 'absolute'
             * */
            if (this.Parent == null)
            {
                this.Width = ParentHost.Width;
                this.Height = ParentHost.Height;
                this.Left = 0;
                this.Top = 0;
            ...
            /*****
             * Assign bounds according to an vbox/hbox
             * */
            ...
            if (_Element.Parent != null)
            {
                ...
                // parentCoordinate´s
                Rectangle coords = new Rectangle(_Element.Parent.Left, _Element.Parent.Top, _Element.Parent.Width, _Element.Parent.Height);
