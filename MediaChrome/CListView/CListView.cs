...
namespace MediaChrome
{
    ...
    public class CListView : Panel
    {
    	...
    	public bool UseCompatibleStateImageBehavior 
    	{
    		get
    		{
    			return Spawn.UseCompatibleStateImageBehavior;
    			
    		}
    		set
    		{
    			Spawn.UseCompatibleStateImageBehavior=value;
    		}
    	}
        ...
        void DrawCore(Graphics g)
        {
            ...
            try
            {
                ...
                int leftx = 0;
                g.FillRectangle(new LinearGradientBrush(new Point(0,0),new Point(0,itemHeight+32),Color.FromArgb(211,211,211),Color.FromArgb(200,200,200)),new Rectangle(0,0,this.Width,itemHeight));
              //  g.DrawImage(MediaChrome.Resources.column, new Rectangle(0, 0, this.Width, ItemHeight));
                for (var x = 0; x < ColumnCount; x++)
                {
                   ...
                   g.DrawLine(new Pen(Color.DarkGray),leftx-2,2,leftx-2,itemHeight);
                   g.DrawLine(new Pen(Color.LightGray),leftx-1,2,leftx-1,itemHeight);
                }
            }
            catch(Exception e)
            {
