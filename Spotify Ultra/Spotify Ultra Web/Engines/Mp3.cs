...
namespace MediaChrome
{
	...
	public class MP3Player : IPlayEngine
	{
        ...
        public Playlist CreatePlaylist( String Name)
        {
            if (Name == "" || Name == null)
                return null;
            String Path = SpofityRuntime.MainForm.DownloadDir + "\\" + Name + ".pls";
            if (!File.Exists(Path))
            {
		...
		{
            SQLiteConnection Conn = SpofityRuntime.MainForm.MakeConnection();
			...
			if(Reader.HasRows)
			{
                ...
                Song result = SpofityRuntime.MainForm.GetSongFromQuery(Reader);
                return result;
     //("mp3:"+Reader.GetString(0));
		...
		public void ImportData(SQLiteConnection Conn,string RootDir){
			...
			foreach(FileInfo X in D){
                    ...
                    SQLiteConnection CP = SpofityRuntime.MainForm.MakeConnection();
        ...
        public List<Song> LoadPlaylist(String PlsID,ref  Playlist playlist)
        {
            ...
            using (StreamReader SR = new StreamReader(SpofityRuntime.MainForm.DownloadDir + "\\" + PlsID))
            {
                String D = "";
                while ((D = SR.ReadLine()) != null)
                {
                    if (D.StartsWith("music:"))
                    {
                       /* Uri Url = new System.Uri(D);
                        ...
                        _Songs.Add(P);*/
                        Song P =SpofityRuntime. MainForm.GetSongFromURI(D);
                    ...
                    else
                    {
                        ...
                        {
                            SQLDR.Read();
                            Song _Song = SpofityRuntime.MainForm.GetSongFromQuery(SQLDR);
		...
		public void AddToPlaylist(string playlistID, Song _Song, int pos)
		{
            try
            {
                String Path = SpofityRuntime.MainForm.GetURIFromSong(_Song);
                if (File.Exists(SpofityRuntime.MainForm.DownloadDir + "\\" + playlistID))
                {
                    List<String> Strings = new List<string>();
                    using (StreamReader SR = new StreamReader(SpofityRuntime.MainForm.DownloadDir + "\\" + playlistID))
                    {
                        String Line = "";
                    ...
                    using (StreamWriter SW = new StreamWriter(SpofityRuntime.MainForm.DownloadDir + "\\" + playlistID))
                    {
